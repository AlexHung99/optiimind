> 1.3.3 採深色青藍聊天版面：左側快捷列與歷史對話、中央聊天卡片、下方靠左輸入與資源列。輸入框下方的「+」選圖片／PDF，旁邊的「截圖」可直接加入圖片預覽；也能拖入檔案。左側不再有重複的附檔按鈕。新對話以日期時間命名，清單完整顯示時間；舊版對話會以問答卡片呈現，空對話也有提示。可用「重新命名」或 F2 修改標題。PDF 文字自動擷取；掃描檔第一頁先依檔案大小縮小並壓縮，亦可按「選頁」指定頁面。下列舊版介面說明以 README.md 的新版操作為準。
...
預設每天檢查一次 GitHub 官方專案的更新清單。下載新版後驗證 SHA-256、版本與檔案路徑。從系統匣選「結束程式」後，以桌面捷徑或 `Start-OptiiChat.cmd` 重新開啟才會套用；單按視窗右上角 × 只是隱藏。啟動器會短暫等待舊程序完全退出，不會在聊天途中重啟。
