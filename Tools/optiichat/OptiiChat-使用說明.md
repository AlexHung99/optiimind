> 1.3.0 已依深色青藍聊天版面重整：左側快捷列與歷史對話、中央聊天卡片、下方輸入與資源列。按輸入框旁的「＋」選圖片／PDF，或直接拖入；首次對話以日期時間命名，可用「重新命名」或 F2 修改。紀錄保存在本機，重啟仍可查看。PDF 文字自動擷取；掃描檔第一頁先依檔案大小縮小並壓縮，亦可按「選頁」指定頁面。下列舊版介面說明以 README.md 的新版操作為準。
