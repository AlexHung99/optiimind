> 1.3.1 已依深色青藍聊天版面重整：左側快捷列與歷史對話、中央聊天卡片、下方輸入與資源列。按輸入框旁的「+」選圖片／PDF，或直接拖入；「截圖」按鈕在「+」右邊，附加圖片會在輸入框顯示縮圖。首次對話以日期時間命名，可用「重新命名」或 F2 修改。紀錄保存在本機，重啟仍可查看。PDF 文字自動擷取；掃描檔第一頁先依檔案大小縮小並壓縮，亦可按「選頁」指定頁面。下列舊版介面說明以 README.md 的新版操作為準。
...
39 項自動測試涵蓋更新驗證／本機修改保護／失敗還原、框選縮放、負座標螢幕定位、截圖取消／附加／传送／暫存清理，以及模型清單、無模型初始化、下載進度／失敗／取消、模型分流、設定保存、Design／Clone 請求、PCM/WAV、主題與系統匣、Unicode 串流與圖片上下文。截圖測試使用合成影像，未讀取真實桌面；空模型下載流程使用模擬清單與串流驗證，沒有刪除現有模型來重下載。Breeze API 音訊測試使用本機模擬服務，並不代表實際 Breeze 模型已驗證。
