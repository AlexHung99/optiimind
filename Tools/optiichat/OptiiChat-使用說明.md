...
「設定 → 下載模型」可開啟 [Ollama 模型庫](https://ollama.com/search)，將想下載的模型名稱貼入後按「下載模型」。下載在背景執行，顯示目前檔案進度，也可以取消；完成後已安裝清單自動更新。要使用新模型，仍須回到「聊天模型」分頁選擇並儲存，原選擇不會被下載動作覆寫。模型大小與硬體需求請先查看模型庫頁面。

新安裝的預設模型為 **llama3.2-vision:latest**，文字與圖片均可使用。已有設定時保留原本選擇；此電腦先前選擇的 Gemma 4 不會被覆寫。原選擇已移除時，改用清單中可用的預設或其他模型。

安裝後首次啟動會初始化模型：若缺少預設的 `llama3.2-vision`，即使本機已有其他模型，也會透過 Ollama 官方模型庫自動下載（約 7.8 GB），顯示進度。已有 OptiChat 設定時保留原本選擇；連線失敗也不會誤判為空清單。下載可停止，失敗或取消後到「設定 → 聊天模型」按「重新整理已安裝模型」再次嘗試，Ollama 可接續已下載內容。

獨立安裝包已包含 Python 與介面套件；首次啟動缺少 Ollama 時會下載官方安裝檔、驗證簽章並自動安裝，再準備 Vision 相容服務。使用 ZIP 原始碼安裝時需先安裝 Python 3.11+，執行 `Install-OptiiChat.cmd` 也會自動補齊 Ollama 與相容執行檔。Llama 3.2 Vision 使用 0.24.0 相容服務；Gemma 等其他模型使用一般 Ollama。兩個清單均取自一般 Ollama 的本機模型庫，與本工具相容服務共用模型目錄。
...
在左側對話紀錄上方按「Agent」分頁，再按「＋ 新任務」。先選擇工作資料夾，輸入想整理或編輯的目標後按「開始任務」。Agent 使用「設定 → 聊天模型」的文字模型，最多執行六個工具步驟。內建文書技能可讀取 PDF 文字層、Excel `.xlsx`、Word `.docx` 和一般文字檔；可替換文字檔中的指定內容、修改 Excel 儲存格，以及替換 Word 單一格式片段中的文字或新增段落。步驟、工具結果及最終答覆都會顯示在執行紀錄；執行中可按「停止」，切回對話分頁不會中斷任務。

Agent 紀錄與一般對話分開存於 `%LOCALAPPDATA%\OptiiChat\agents`，左側可開啟舊任務，按右鍵可重新命名或刪除。每次文件編輯都會先顯示變更預覽，待你確認後才在所選資料夾另存新檔，原檔不覆蓋；取消時不寫入。PDF 在 Agent 中僅讀取可擷取的文字，掃描 PDF 請到聊天分頁選頁交給圖片模型。單份 Excel、Word 或文字文件上限 20 MB，PDF 上限 100 MB；預覽和工具輸出會限量。未選擇資料夾時可請模型直接回答，但沒有檔案工具。不執行命令，也不包含 MCP 或外部 Skill 整合。

## 設定

右上角齒輪的「設定」有外觀、聊天模型、下載模型與語音等頁；符合單卡 NVIDIA 12 GB 顯存建議配置才顯示 Breeze 引擎與服務參數頁。無法偵測硬體時也會隱藏。
...
預設為 **Windows 本機語音**。下方的手動朗讀、停止語音與匯出 WAV 按鈕目前隱藏；需要語音時仍可在設定勾選「模型完成回覆後自動朗讀」。
